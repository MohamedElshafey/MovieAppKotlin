 ...
 */
class PopularManager(private var retrofit: Retrofit) {
    private var mUiHandler = Handler()

    fun online(page: Int): Observable<Popular> {
        val retrofitService = retrofit.create(RetrofitService::class.java)

        return retrofitService.listPopularMovies(MovieDbAPiConst.apiKey, page)
    ...
    fun insertMovieDataInDb(movieDataBase: MovieDatabase, mDbWorkerThread: DbWorkerThread, movie: ArrayList<Movie>) {
        val task = Runnable {
            movie.forEach { movieDataBase.movieDAO().insert(it) }
        }
        mDbWorkerThread.postTask(task)
    }
